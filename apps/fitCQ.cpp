...
struct FitResults {
    ...
    void print_results();
    void print_status();
    void write_results(std::ostream& outfile, double KTmin, double KTmax);
};
...
FitResults fit_CoulCorrLevy(TH1D* hist, double Qmin, double Qmax);
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            Qmin = std::stod(argv[++i]);
        } else if (arg == "--Qmax") {
            Qmax = std::stod(argv[++i]);
        }
    }
    // --- Parsing finished ---
    ...
    for(int i = startFrom; i < fit+startFrom; i++) {
        std::cerr << "Fitting hist " << i << "...\n";
        FitResults res = fit_CoulCorrLevy(
            KThists.Chists.at(i), Qmin, Qmax
        );
        res.print_results();
        std::cerr << "-------------------------------------------" << std::endl;
        res.write_results(
...
void FitResults::write_results(
    std::ostream& outfile, double KTmin, double KTmax
) {
    outfile << '\n' << KTmin << '\t' << KTmax << '\t' << success << '\t';
    for(int i = 0; i < NPARS; i++) {
        outfile << params.at(i) << "\t";
